...
const Signup = ({ navigation }) => {
    ...
    const handleSignup = async (email, password) => {
        try {
            ...
            USERS.doc(customer.email).onSnapshot(async u => {
                if (!u.exists) {
                    await auth().createUserWithEmailAndPassword(customer.email, customer.password)
                        .then(() => USERS.doc(customer.email).set(customer).then(() => { console.log("Add new user customer!"); navigation.navigate("Signin") }))
                }
            })
        } catch (error) {
            console.log(error)
